    ...
    React.useEffect(() => {
        ...
        const scrollHandler = () => {
            ...
            for (let a of document.querySelectorAll('.toc a')) {
                // @ts-ignore
                const bounding = document.getElementById(decodeURI(a.hash.slice(1))).getBoundingClientRect();
    ...
    return (React.createElement("section", { className: "main" },
        React.createElement("div", { className: "main_article" },
            ...
            (prev || next) && (React.createElement("div", { className: "prev_next" },
                prev && (React.createElement("a", { className: "prev button", href: `${prev.link}` },
                    "\u00AB\u00A0\u00A0",
                    prev.text)),
                next && (React.createElement("a", { className: "next button", href: `${next.link}` },
                    next.text,
                    "\u00A0\u00A0\u00BB")))),
            gitalk),
